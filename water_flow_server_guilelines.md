...
Bytes 11–12. Generated by the server, must not repeat. The protocol says the meter
echoes the same number in its reply and that this is how you correlate a reply with
the command that caused it.

**Do not correlate on it — this firmware does not honour it.** An error is answered
with *two* frames: a generic one carrying instruction number `0000` and data
identifier `0000`, then the real one carrying the number and identifier you sent.
Matching on the number credits the generic frame to the command via whatever fallback
you built, and then the real reply matches nothing. Worse, the generic frame has been
observed arriving *before* any command was sent on the connection, so a fallback of
the form "if exactly one command is outstanding, assume it is that one" will attribute
it to a command from an earlier session.

Correlate on the session instead. The meter dials in, exchanges, and powers down, so
the command in flight on the socket is the one being answered; a reply with nothing in
flight belongs to nobody. Record only the first verdict, since the second frame will
arrive carrying the same error. Keep generating non-repeating numbers for the wire —
nothing is known about whether the firmware deduplicates on them — but nothing needs
to store or index them. Observed on `65.1.99.130`, 2026-08-04.
...
- [ ] Unique, non-repeating instruction numbers on the wire — but match replies to
      commands by session, not by number
